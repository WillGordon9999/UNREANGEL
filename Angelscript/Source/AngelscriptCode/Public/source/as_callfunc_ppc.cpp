...
asQWORD CallSystemFunctionNative(asCContext *context, asCScriptFunction *descr, void *obj, asDWORD *args, void *retPointer, asQWORD &/*retQW2*/, void */*secondObject*/)
{
	...
	if( !sysFunc->takesObjByVal )
	{
		...
		{
			if( descr->parameterTypes[s].IsFloat32Type() && !descr->parameterTypes[s].IsReference() )
			{
				argsType[a] = ppcFLOATARG;
			}
			else if( descr->parameterTypes[s].IsFloat64Type() && !descr->parameterTypes[s].IsReference() )
			{
	...
	if( sysFunc->takesObjByVal )
	{
		...
		for( asUINT n = 0; n < descr->parameterTypes.GetLength(); n++ )
		{
			...
			else
			{
				...
				paramBuffer[dpos++] = args[spos++];
				if( descr->parameterTypes[n].IsFloat32Type() && !descr->parameterTypes[n].IsReference() )
					argsType[a++] = ppcFLOATARG;
				else if( descr->parameterTypes[n].IsFloat64Type() && !descr->parameterTypes[n].IsReference() )
					argsType[a++] = ppcDOUBLEARG;
				else
					argsType[a++] = ppcINTARG;
				...
				{
					paramBuffer[dpos++] = args[spos++];
					if( !descr->parameterTypes[n].IsFloat64Type() ) // Double already knows it is 2 dwords
						argsType[a++] = ppcINTARG;
				}
